...
import com.ds.ads.backend.AdsApplication;
import com.ds.ads.backend.services.LocationRepository;
...
public class LoginCtrlTest {
	...
	long oldValue = userRepository.count();
	...
	assertThat("userRep count now +1", userRepository.count(), equalTo(oldValue + 1));
    }
	...
	User uFromRepo = userRepository.findByLogin((String) map.get("login"));
	Assert.notNull(uFromRepo, "new user in repo exists");
	Assert.notNull(uFromRepo.getLocation(), "new users location in repo exists");
	MatcherAssert.assertThat("location in repo equals to given location", uFromRepo.getLocation().equals(loc));
    }
